...
public static class TypesCommand
{
    public static Command Build()
    {
        ...
        var noBindingsOption = new Option<bool>(
            "--no-bindings", "Do not emit callable invoke() wrappers (carbon.gen.ts)");

        command.AddOption(projectOption);
        command.AddOption(outOption);
        command.AddOption(noCapabilitiesOption);
        command.AddOption(noBindingsOption);
        command.SetHandler(Run, projectOption, outOption, noCapabilitiesOption, noBindingsOption);
        return command;
    }

    private static void Run(DirectoryInfo? projectDir, FileInfo? outFile, bool noCapabilities, bool noBindings)
    {
        var root = projectDir?.FullName ?? Directory.GetCurrentDirectory();
        var result = Generate(root, outFile?.FullName, !noCapabilities, !noBindings);

        Console.ForegroundColor = ConsoleColor.Green;
        Console.WriteLine($"⚡ Generated {result.CommandCount} command type(s) → {result.TargetPath}");
        Console.ResetColor();
        if (result.BindingsPath is not null)
            Console.WriteLine($"⚡ Generated invoke() wrappers → {result.BindingsPath}");
        ...
        bool emitBindings = true)
    {
        ...
        var ordered = commands.OrderBy(c => c.Name).ToList();
        var dts = Emit(ordered, plugins, records);
        ...
        // Alongside the declarations, emit callable typed invoke() wrappers (Tauri ships these too).
        string? bindingsPath = null;
        if (emitBindings)
        {
            bindingsPath = Path.Combine(targetDir ?? root, "carbon.gen.ts");
            File.WriteAllText(bindingsPath, EmitBindings(ordered));
        }
    ...
    /// <summary>
    /// Emits callable, typed <c>invoke()</c> wrappers grouped by plugin namespace, e.g.
    /// <c>app.greet(args)</c>. Argument and result types reuse the declarations in carbon.d.ts via
    /// indexed access on <c>CarbonCommands</c>, so the wrappers stay in sync and don't duplicate shapes.
    /// </summary>
    private static string EmitBindings(List<CarbonCommandInfo> commands)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// AUTO-GENERATED by `carbon types` — do not edit.");
        // Pull in the command declarations so CarbonCommands is populated when this file is compiled.
        sb.AppendLine("/// <reference path=\"./carbon.d.ts\" />");
        sb.AppendLine("import { invoke } from '@dotcarbon/api';");
        sb.AppendLine("import type { CarbonCommands } from '@dotcarbon/api';");

        foreach (var group in commands.GroupBy(command => command.Namespace).OrderBy(group => group.Key))
        {
            sb.AppendLine();
            sb.AppendLine($"export const {group.Key} = {{");
            foreach (var command in group.OrderBy(command => command.LocalName))
            {
                var key = IsJsIdentifier(command.LocalName) ? command.LocalName : $"'{command.LocalName}'";
                var result = $"CarbonCommands['{command.Name}']['result']";
                sb.AppendLine($"    /** {command.Name} */");
                if (command.ArgType is null)
                    sb.AppendLine($"    {key}: (): Promise<{result}> => invoke('{command.Name}'),");
                else
                    sb.AppendLine(
                        $"    {key}: (args: CarbonCommands['{command.Name}']['args']): Promise<{result}> => " +
    ...
    private static bool IsJsIdentifier(string value) =>
        value.Length > 0 &&
